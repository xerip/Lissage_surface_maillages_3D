...
void Courbures::trianguler(MyMesh::VertexHandle sommets, int size)
{
    ;
}

int Courbures::draw_quad(MyMesh::VertexHandle vh, MyMesh *mesh)
{
//    qDebug() << "<" << __FUNCTION__ << ">";
...
//    Eigen::Translation3d t(p_e[0], p_e[1], p_e[2]) ;
    ...
    if (neigh.size() >= 5)
    {
        ...
        for(int i=0; i<neigh.size(); i++)
        {
            MyMesh::VertexHandle myVh = neigh.at(i);

            mesh->data(myVh).thickness = 5;
            mesh->set_color(myVh, MyMesh::Color(0, 255, 0));
...
//            sommets[i] = mesh->add_vertex(MyMesh::Point(tmpp[0], tmpp[1],  tmpp[2]));
            sommets[i] = mesh->add_vertex(MyMesh::Point(tmpp[0], tmpp[1],  tmpp[2]));
//            sommets[i] = mesh->add_vertex(p1);
            mesh->data(sommets[i]).thickness = 5;
            mesh->set_color(sommets[i], MyMesh::Color(255, 0, 0));
...
//        qDebug() << "neigh.size(): " << neigh.size();
        int cols = 4;
        int lines = neigh.size()/cols;
        std::vector<MyMesh::VertexHandle> uneNouvelleFace;
        FaceHandle fh;


        /* JOJO for (int i=0; i<cols-1; i+=1)
        {
            for (int j=0; j<lines-1; j+=1)
            {
                uneNouvelleFace.clear();
                uneNouvelleFace.push_back(sommets[i+j]);
                uneNouvelleFace.push_back(sommets[i+lines+j]);
                uneNouvelleFace.push_back(sommets[i+lines+1+j]);
                mesh->add_face(uneNouvelleFace);
                fh = mesh->face_handle(mesh->n_faces()-1);
                mesh->set_color(fh, MyMesh::Color(0, 0, 255));
                for (MyMesh::FaceEdgeIter fe_it=mesh->fe_iter(fh); fe_it.is_valid(); fe_it++)
                {
                    EdgeHandle eh = *fe_it;
                    mesh->data(eh).thickness = 1;
                    mesh->set_color(eh, MyMesh::Color(0, 100, 255));
                }

//                uneNouvelleFace.clear();
//                uneNouvelleFace.push_back(sommets[i+j]);
//                uneNouvelleFace.push_back(sommets[i+j+1]);
//                uneNouvelleFace.push_back(sommets[i+lines+j+1]);
//                mesh->add_face(uneNouvelleFace);
//                fh = mesh->face_handle(mesh->n_faces()-1);
//                mesh->set_color(fh, MyMesh::Color(0, 0, 255));
//                for (MyMesh::FaceEdgeIter fe_it=mesh->fe_iter(fh); fe_it.is_valid(); fe_it++)
...
//        for (int i=0; i<neigh.size()-3; i+=1)
//        {
//            uneNouvelleFace.clear();
//            uneNouvelleFace.push_back(sommets[i]);
//            uneNouvelleFace.push_back(sommets[i+1]);
//            uneNouvelleFace.push_back(sommets[i+2]);
//            mesh->add_face(uneNouvelleFace);
//            FaceHandle fh = mesh->face_handle(mesh->n_faces()-1);
//            mesh->set_color(fh, MyMesh::Color(0, 0, 255));
//        }

        /*
        VertexHandle vh2 = sommets[(neigh.size()/2) - 1];
        VertexHandle vh3 = sommets[neigh.size()/2];
        uneNouvelleFace.clear();
        uneNouvelleFace.push_back(vh);
        uneNouvelleFace.push_back(vh2);
        uneNouvelleFace.push_back(vh3);
        uneNouvelleFace.push_back(mesh->vertex_handle(2));
        mesh->add_face(uneNouvelleFace);

        mesh->data(vh).thickness = 8;
        mesh->set_color(vh, MyMesh::Color(0, 255, 255));
        mesh->data(vh2).thickness = 8;
        mesh->set_color(vh2, MyMesh::Color(0, 255, 255));
        mesh->data(vh3).thickness = 8;
        mesh->set_color(vh3, MyMesh::Color(0, 255, 255));
        mesh->data(mesh->vertex_handle(2)).thickness = 8;
        mesh->set_color(mesh->vertex_handle(2), MyMesh::Color(0, 255, 255));

        for (int j=0; j<mesh->n_faces()-1; j++)
        {
            FaceHandle fh = mesh->face_handle(j);
            mesh->set_color(fh, MyMesh::Color(0, 0, 255));
        }
//        FaceHandle fh = mesh->face_handle(mesh->n_faces());
        */
